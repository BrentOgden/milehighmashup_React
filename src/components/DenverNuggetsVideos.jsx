// src/components/DenverNuggetsVideos.jsx
import React, { useState, useEffect } from 'react';
import espn from '../api/espnApi';

const TEAM_ID = 7;

export default function DenverNuggetsVideos() {
  ...
  useEffect(() => {
    espn.get('/sports/basketball/nba/news', { params: { lang:'en', region:'us', limit:10 } })
      .then(res => {
      ...
      .catch(() => setError('Failed to load Nuggets clips'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <p className="text-center text-gray-500">Loading Nuggets…</p>;
  if (error)   return <p className="text-center text-red-500">{error}</p>;
  if (!clips.length) return <p className="text-center text-gray-500">No Nuggets clips.</p>;
            ...
            <div className="flex flex-col h-full bg-white shadow-lg rounded-2xl overflow-hidden">
              {thumb && (
                <img
                  src={thumb}
                  alt={clip.headline}
                  className="w-full md:h-48 object-cover flex-shrink-0"
                />
              )}
