// src/components/DenverBroncosLastGame.jsx
...
export default function DenverBroncosLastGame() {
  ...
  // format date as "MM-DD" in Denver time
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        // 1) Last completed game (reg + playoffs)
        const [regRes, postRes] = await Promise.all([
          espn.get(`/sports/football/nfl/teams/${TEAM_ID}/schedule`, {
        ...
        const events = [...(regRes.data.events||[]), ...(postRes.data.events||[])];
        const completed = events
          .filter(e => e.competitions?.[0]?.status?.type.completed)
          .sort((a,b) =>
            new Date(b.competitions[0].date) - new Date(a.competitions[0].date)
          );
        if (!completed.length) throw new Error('No completed games found');
        ...
        if (overall) {
          // W–L record
          setRecordSummary(overall.displayValue);  // e.g. "10-7"

          // Win %
          const pctStat = overall.stats.find(s => s.type === 'winpercent');
          if (pctStat) {
            setWinPct(`${(pctStat.value * 100).toFixed(1)}%`); // e.g. "58.8%"
          }

          // Playoff seed
          const seedStat = overall.stats.find(s => s.type === 'playoffseed');
          if (seedStat) {
            const seed = seedStat.displayValue && seedStat.displayValue !== '-'
        ...
        setError(
          e.message.includes('No completed')
            ? 'No completed Broncos games found.'
            : 'Failed to load Broncos data.'
        );
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="max-w-sm mx-auto bg-white shadow rounded-lg p-6 text-center">
        <p className="text-gray-500">Loading last Broncos game…</p>
      </div>
    );
  }
  if (error || !game) {
