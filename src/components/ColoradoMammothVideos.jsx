// src/components/ColoradoMammothVideos.jsx
import React, { useState, useEffect } from 'react';
import espn from '../api/espnApi';

const TEAM_ID = 125422;

export default function ColoradoMammothVideos() {
  ...
  useEffect(() => {
    espn.get('/sports/lacrosse/nll/news', { params: { lang:'en', region:'us', limit:10 } })
      .then(res => {
      ...
      .catch(() => setError('Failed to load Mammoth clips'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <p className="text-center text-gray-500">Loading Mammoth</p>;
  if (error)   return <p className="text-center text-red-500">{error}</p>;
  if (!clips.length) return <p className="text-center text-gray-500">No Mammoth clips.</p>;
