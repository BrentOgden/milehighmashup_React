// src/components/ColoradoMammothLastGame.jsx
...
export default function ColoradoMammothLastGame() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        // 1) Last completed Mammoth game (regular + playoffs)
        const [regRes, postRes] = await Promise.all([
          espn.get(`/sports/lacrosse/nll/teams/${TEAM_ID}/schedule`, {
        ...
        if (overall) {
          // Record summary
          setRecordSummary(overall.displayValue); // e.g. "8-10"

          // Win %
          const pctStat = overall.stats.find(s => s.type === 'winpercent');
          if (pctStat) {
            // displayValue is ".444" → convert to "44.4%"
            const raw = parseFloat(pctStat.displayValue);
            setWinPct(`${(raw * 100).toFixed(1)}%`);
          }

          // Playoff seed (stat.type is lowercase "playoffseed") :contentReference[oaicite:2]{index=2}
          const seedStat = overall.stats.find(s => s.type === 'playoffseed');
          if (seedStat) {
            setPlayoffSeed(seedStat.displayValue || seedStat.value.toString());
          }
        }
      } catch (e) {
        ...
        setError(
          e.message.includes('No completed')
            ? 'No completed Mammoth games found.'
            : 'Failed to load Mammoth data.'
        );
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="max-w-sm mx-auto bg-white shadow rounded-lg p-6 text-center">
        <p className="text-gray-500">Loading last Mammoth game…</p>
      </div>
    );
  }
  if (error || !game) {
    ...
    <div className="mx-4 bg-white shadow-lg rounded-2xl h-fit overflow-hidden">
      <div className="p-6">
        ...
        <div className="flex items-center justify-center space-x-4 mb-4">
          <img
            src={getLogo(awayComp)}
            alt={awayComp.team.displayName}
            className="h-12 w-12 object-contain"
          />
          <span className="text-black font-medium">vs</span>
          <img
            src={getLogo(homeComp)}
            alt={homeComp.team.displayName}
            className="h-12 w-12 object-contain"
          />
        </div>
      ...
      <div className="text-center mb-4">
        ...
        {playoffSeed && (
          <p className="text-black">#{playoffSeed} in the NLL</p>
        )}
      </div>
    </div>
  );
