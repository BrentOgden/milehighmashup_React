// src/components/ColoradoAvalancheVideos.jsx
import React, { useState, useEffect } from 'react';
import espn from '../api/espnApi';

const TEAM_ID = 17;

export default function ColoradoAvalancheVideos() {
  ...
  useEffect(() => {
    espn.get('/sports/hockey/nhl/news', { params: { lang:'en', region:'us', limit:10 } })
      .then(res => {
      ...
      .catch(() => setError('Failed to load Avalanche clips'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <p className="text-center text-gray-500">Loading Avalanche…</p>;
  if (error)   return <p className="text-center text-red-500">{error}</p>;
  if (!clips.length) return <p className="text-center text-gray-500">No Avalanche clips.</p>;
